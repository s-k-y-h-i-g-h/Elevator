...
/// <summary>
/// Platform-specific secure storage service for JWT tokens
/// </summary>
public class SecureTokenStorage
{
    ...
    public async Task SaveTokenAsync(string token, DateTime expiresAt, string userEmail)
    {
        try
        {
            await SecureStorage.SetAsync(TokenKey, token);
            await SecureStorage.SetAsync(ExpirationKey, expiresAt.ToBinary().ToString());
            await SecureStorage.SetAsync(UserEmailKey, userEmail);
        }
        catch (Exception ex)
        {
            // Log the exception but don't throw to avoid breaking the auth flow
    ...
    public static bool IsSecureStorageAvailable()
    {
        try
        {
            // Test if secure storage is available by attempting a simple operation
            return SecureStorage.Default != null;
        }
        catch (Exception)
        {
            return false;
